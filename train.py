...
def main():
    ...
    model = load_pretrained_model(in_arg.arch)
    
    # Trains the model and prints running and training loss
    trained_model = train_model(model, data[3], in_arg.learn_rate, in_arg.epochs, in_arg.gpu)
    
    # Tests the model for test images (From test data set) and prints accurcy
    test_network_model(trained_model[0], data[5])
    
    # Save the Model Checkpoint
    #save_model_checkpoint(model, epochs, optimizer, train_datasets):
    save_model_checkpoint(trained_model[0], in_arg.arch, in_arg.epochs, trained_model[2], data[0])    
...
def load_pretrained_model(arch):
    ...
    mymodel = pm.load_pretrained_model(arch)
    return mymodel

def train_model(model, train_data, learn_rate, nepochs, gpu):
    ...
    modeltotrain = model
    print(modeltotrain.classifier)
    
    criterion = nn.NLLLoss()
    optimizer = optim.Adam(modeltotrain.classifier.parameters(), lr = lr)
    print(lr)
    print(optimizer)
        ...
        for ii, (inputs, labels) in enumerate(trainloader):
            ...
            if steps % print_every == 0:
                ...
                print("Epoch : {}/{}...".format(e+1, epochs),
                      ...
                      "Validation Loss: {:.3f}.. ".format(test_loss/len(trainloader)))
...
def test_network_model(model, test_data):
    ...
    with torch.no_grad():
        ...
        print('Accuracy of the network on the test images: %d %%' % (100 * accuracy / total))    
        return accuracy

def save_model_checkpoint(model, arch, epochs, optimizer, train_datasets):
    
    # Following is ond of the methogs to save without saving other details of the model
    # torch.save(vgg16model.state_dict(), 'vgg16model_checkpoint.pth')    
    ...
    if arch == 'vgg16':
        ...
        print('VGG16 Model Checkpoint saved')
    elif arch == 'alexnet':
        torch.save(checkpoint, 'alexnetcheckpoint.pth') 
        print('Alexnet Checkpoint saved')
